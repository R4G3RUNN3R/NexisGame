...
export type TravelFinalizeResult = {
...
type TravelRuntimeContextValue = {
  ...
  startTravel: (
    destinationCityId: string,
    durationMs: number,
    options?: { riskChance?: number; failureMessage?: string; successMessage?: string },
  ) => boolean;
  finalizeTravel: () => TravelFinalizeResult | null;
  resetOutcome: () => void;
...
export function TravelRuntimeProvider({ children }: { children: React.ReactNode }) {
        ...
        setTravelState((prev) => ({
          ...
          lastOutcome: JSON.stringify({
            ...
            failureMessage:
              ...
              "You lost your way and returned to your origin.",
            successMessage:
              options?.successMessage ?? "Journey completed successfully.",
          }),
        }));
        return true;
      },
      finalizeTravel() {
        if (!travelState.destinationCityId) return null;

        const metadata = travelState.lastOutcome
          ? (JSON.parse(travelState.lastOutcome) as {
              riskChance: number;
        ...
        const result: TravelFinalizeResult = {
          status: failed ? "failed" : "arrived",
          message: failed
            ? metadata?.failureMessage ??
              "You lost your way and returned to your origin."
            : metadata?.successMessage ?? "Journey completed successfully.",
          currentCityId: failed
            ? travelState.currentCityId
            : travelState.destinationCityId,
          destinationCityId: travelState.destinationCityId,
        };

        setTravelState({
          currentCityId: result.currentCityId,
          destinationCityId: failed
            ? travelState.currentCityId
            : travelState.destinationCityId,
          startedAt: null,
          completesAt: null,
  ...
  return (
    <TravelRuntimeContext.Provider value={value}>
      {children}
...
export function useTravelRuntime() {
  ...
  if (!ctx)
    throw new Error("useTravelRuntime must be used within a TravelRuntimeProvider");
  return ctx;
}
